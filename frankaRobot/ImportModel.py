...
class RNNSequenceFeatureByTime(nn.Module):
    def __init__(self, network_type, num_classes = 5, num_features=4, time_window=200) :
        super(RNNSequence, self).__init__()
        if network_type == 'LSTM':
            hidden_size = 50
            self.innernet = nn.LSTM(input_size=time_window, hidden_size=hidden_size, num_layers=1, batch_first=True)
        elif network_type == 'GRU':
            hidden_size = 50
            self.innernet = nn.GRU(input_size=time_window, hidden_size=hidden_size, num_layers=1, batch_first=True)
        elif network_type == 'FCLTC':
            units = 50
            self.innernet = LTC(input_size=time_window, units=units, batch_first=True)
        elif network_type == 'FCCfC':
            units = 50
            self.innernet = CfC(input_size=time_window, units=units, batch_first=True)
        elif network_type == 'NCPLTC':
            units = 53
...
def import_rnn_models_FeatureByTime(PATH:str, network_type:str, num_classes:int,  num_features:int, time_window:int):

	model = RNNSequenceFeatureByTime(network_type = network_type, num_classes = num_classes, num_features=num_features, time_window=time_window)
	checkpoint = torch.load(PATH)
	model.load_state_dict(checkpoint["model_state_dict"])
