""" 
rho的代码:两个X之间的similarity
        

v0的代码：
def set_v0(self, X):
        feature_variances = np.std(X, axis=0) # 计算每个特征的标准差
        self.v0 = np.max(feature_variances)  

表现：
P: 0.98 ,0.97 ,0.97 ,0.98 ,0.99
G: 1 ,0.99 ,0.97 ,0.99 ,0.96
ST: 0.99 ,1 ,1 ,0.98 ,1
--------
v0的代码：
def set_v0(self, X):
...
表现：
P:0.99,0.96,0.97,0.98,0.98
G:0.99,0.99,0.99,0.99,0.96
ST:0.99,1,1,0.98 ,0.99
...
class RBFNetwork(object):
    ...
    def fit(self, X, y):
        ...
        print(f"upper__triangular is {upper_triangular}")
        print(f"length of upper_triangular is {len(upper_triangular)}")
        print(f"length of y is {len(y)}")
        rho_min = np.percentile(upper_triangular, 10)
        rho_max = np.percentile(upper_triangular, 90)
        print(f"rho_min is {rho_min}")
        print(f"rho_max is {rho_max}")
        # 选择一个合适的rho值，例如取范围的均值
        self.rho = (rho_min + rho_max) / 2
        print(f"Dynamic rho set to: {self.rho}")  # 输出动态设置的 rho

        # Create the y comparison matrix
        y_comparison_matrix = np.ones((len(y), len(y))) * -1  # Initialize with -1
        for i in range(len(y)):
            for j in range(i, len(y)):
                if y[i] == y[j]:
                    y_comparison_matrix[i, j] = 1
                y_comparison_matrix[j, i] = y_comparison_matrix[i, j] 

        upper_triangular_similarity = similarity_matrix[np.triu_indices_from(similarity_matrix, k=1)]
        upper_triangular_y_comparison = y_comparison_matrix[np.triu_indices_from(y_comparison_matrix, k=1)]
         # Step 4: Save to CSV
        df = pd.DataFrame({
            'Similarity': upper_triangular_similarity,
            'Y_Comparison': upper_triangular_y_comparison
        })
        df.to_csv("similarity_y_comparison.csv", index=False)
        ...
        # Now, save the upper_triangular_values and corresponding predictions to a CSV file
