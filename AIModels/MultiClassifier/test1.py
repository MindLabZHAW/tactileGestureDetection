""" 
这个根据数据自动找rho值的代码，rho取的是平均值，对ST特别友好，能达到100%：
        similarity_percentiles = np.percentile(all_similarities, [10, 90])
        rho_range_min, rho_range_max = similarity_percentiles  # 可以选择这个范围
        self.rho = (rho_range_min + rho_range_max) / 2
P: 0.97,0.91,0.92,0.95,0.98 /V0= 10, 能达到0.98,0.96,0.97,0.98,0.99
G: 0.98,0.90,0.94,0.95,0.91/V0= 10, 能达到0.99,0.99,0.98,0.98,0.95
ST:0.99,0.99,1,0.96,0.99/ V0= 10, 能达到1
    ...
    def fit(self, X, y):
        ...
        for x in X:
            for idx, center in enumerate(self.centers):
                ...
                # print(f"input_sim is {input_sim}")
                all_similarities.append(input_sim)
        # print(f"all_similarities is {all_similarities}")
