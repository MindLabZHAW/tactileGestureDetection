...
class Gesture(object):
    ...
    def classifier_train(self):
        ...
        # plot_clusters(self.X_train, self.y_train, self.gesture_model.centers,self.gesture_model.cluster_labels)
        distance_matrix = calculate_distance_matrix(self.gesture_model.centers,self.gesture_model.cluster_labels)
    ...
    def classifier_test(self):
        ...
        # plot_clusters(self.X_test, self.y_test,self.gesture_model.centers,self.gesture_model.cluster_labels, predictions=self.y_predict)
...
def plot_clusters(X, y, centers,cluster_labels, predictions=None, plot_centers=True):
    ...
    for i, label in enumerate(unique_labels):
        plt.scatter(X_reduce[y == label, 0], X_reduce[y == label, 1], label=f"Cluster {label}", color=cluster_colors[i], alpha=0.6)

    if predictions is not None:
        for i, label in enumerate(np.unique(predictions)):
            plt.scatter(X_reduce[predictions == label, 0], X_reduce[predictions == label, 1], marker='x', label=f"Predicted {label}", color=cluster_colors[i], alpha=0.6)
...
def calculate_distance_matrix(centers,cluster_labels):
    centers = np.array(centers)
    ...
    labels = [f"Center {i} (Label {label})" for i, label in enumerate(cluster_labels)]
    distance_matrix_df = pd.DataFrame(distance_matrix, index=labels, columns=labels)
